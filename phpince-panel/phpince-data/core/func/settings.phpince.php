...
if($PHPINCE_perms["systeminfo"]){
	...
	if(!empty($_POST)){
		...
		(empty($_POST["site_key"]))){
			...
			if((!preg_match("|^[0-9\p{L}_\s-!?,.]*$|u", $_POST["site_name"]))||
			(!preg_match("|^[0-9\p{L}_\s-!?,.]*$|u", $_POST["site_desc"]))||
			(!preg_match("|^[0-9\p{L}_\s-!?,.]*$|u", $_POST["site_key"]))){
				echo "<div class=\"warn no\"><img src=\"/phpince-panel/phpince-data/core/tems/phpince-dashboard/icon/warn_no.png\">".$PHPINCE_LANG[519]."</div>";
			} else {
				...
				bl_redirect("/panel/settings", 2, $PHPINCE_LANG);
			}
		}
	}
